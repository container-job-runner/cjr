...
export default class Run extends StackCommand {
  ...
  async run()
  {
    ...
    if(flags.url || (flags.start && result.success && !jupyter_app)) // -- list jupyter url ---------------------------------------
    {
      result = await getJupyterUrl(container_runtime, stack_path, {"job-id": args['id']})
      if(result.success) console.log(result.data)
    }
    if(flags.app || (flags.start && result.success && jupyter_app)) // -- start electron app -------------------------------------
    {
      result = await getJupyterUrl(container_runtime, stack_path, {"job-id": args['id']})
