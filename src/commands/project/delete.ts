...
export default class Delete extends ProjectSettingsCommand {
  ...
  static flags = {
    ...
    "config-files": flags.boolean({multiple: true, description: "remove any additional overriding configuration files for project stack"}),
    "stacks-dir": flags.boolean({description: "remove any overriding default stack directory for project"}),
    "visible-stacks": flags.boolean({multiple: true, description: "if specified only these stacks will be affected by this command"}),
  }
  static strict = false;
  ...
  async run()
  {
    ...
    const project_root = prflag['project-root']
    // -------------------------------------------------------------------------
